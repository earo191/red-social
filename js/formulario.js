...
var validarRadios = function(){
	var opciones = document.getElementsByName('sexo'),
        opciones_2 = document.getElementsByName('condicion'),
        resultado_2 = false;
		resultado = false;
	...
	for (var i = 0; i < elementos.length; i++) {
		if(elementos[i].type == "radio" && elementos[i].name == "condicion"){
			// Recorremos los radio button
			for (var o = 0; o < opciones_2.length; o++) {
				if (opciones_2[o].checked) {
					resultado_2 = true;
					break;
				}
			}
			...
			if (resultado_2 == false) {
				elementos[i].parentNode.className = elementos[i].parentNode.className + " error";
				console.log('El campo condicion esta incompleto');
				return false;
			} else {
				// Eliminamos la clase Error del radio button
				elementos[i].parentNode.className = elementos[i].parentNode.className.replace(" error", "");
